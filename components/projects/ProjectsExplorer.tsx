...
type Filter = 'all' | 'ai-workflow' | 'ai-application' | 'ai-driven-development';
...
const labels = {
  ja: {
    ...
    introBody: '公開できる範囲の実績を厳選して掲載しています。ワークフロー系や開発体制の事例は、守秘性の高い業務要件を含むため、代表例として構造化して紹介します。',
    ...
    drivenEyebrow: 'AI-Driven Development',
    drivenTitle: 'AI駆動開発の代表実績',
    drivenSupport: 'AIエージェントを組み込んだ開発プロセスと品質ゲートにより、短いサイクルで検証・改善を回す開発体制そのものを示します。',
  ...
  en: {
    ...
    introBody: 'We show a focused set of public proof. Workflow-oriented and delivery-methodology cases often involve sensitive operational logic, so they are presented here as structured representative examples.',
    workflowEyebrow: 'AI Workflow Design & Development',
    ...
    drivenEyebrow: 'AI-Driven Development',
    drivenTitle: 'Representative AI-driven development proof',
    drivenSupport: 'This demonstrates a delivery system that uses AI-agent-assisted development processes and quality gates to run short validation and improvement cycles.',
...
export default function ProjectsExplorer({ lang }: ProjectsExplorerProps) {
  ...
  const drivenProjects = featuredProjects.filter((project) => project.serviceLine === 'ai-driven-development');

  const showWorkflow = filter === 'all' || filter === 'ai-workflow';
  const showAi = filter === 'all' || filter === 'ai-application';
  const showDriven = filter === 'all' || filter === 'ai-driven-development';
  ...
  return (
    <div className={styles.sectionStack}>
      ...
      {showDriven ? (
        <section className={styles.sectionStack}>
          <div className={styles.sectionHeader}>
            <span className={styles.sectionEyebrow}>{t.drivenEyebrow}</span>
            <h2 className={styles.sectionTitle}>{t.drivenTitle}</h2>
            <p className={styles.sectionSubtitle}>{t.drivenSupport}</p>
          </div>
          <div className={styles.grid2}>
            {drivenProjects.map((project) => (
              <div key={project.id} className={`${styles.proofCard} ${styles.featuredCard}`}>
                <span className={styles.proofLabel}>{t.drivenEyebrow}</span>
                <h3 className={styles.proofTitle}>{project.name[lang]}</h3>
                <p className={styles.proofSummary}>{project.summary[lang]}</p>
                <div className={styles.proofStrip}>
                  {project.proofPoints.map((point: string) => (
            ...
            <div className={styles.card}>
              <h3 className={styles.cardTitle}>{t.drivenTitle}</h3>
              <p className={styles.cardBody}>{t.drivenSupport}</p>
            </div>
          </div>
        </section>
      ) : null}
