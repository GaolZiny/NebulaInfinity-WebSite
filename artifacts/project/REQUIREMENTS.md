...
- [ ] The new `Agentic AI 開発ワークフロー` methodology/case-detail page must be positioned under AI Workflow / multi-agent product delivery workflow, not as a Home Project route and not as AI-Driven Development proof
...
- [ ] AI Workflow representative example card `Agentic AI 開発ワークフロー` links to `/[lang]/services/ai-workflow/ai-dev-flow` with CTA `開発ワークフロー詳細` / aligned EN CTA
...
- [ ] `/[lang]/services/ai-workflow/ai-dev-flow` exists as a static-exported AI Workflow service sub-detail / methodology page
...
- [ ] Basic SEO is in place (title, description, OGP, sitemap) and sitemap excludes removed standalone index routes and retired detail/project routes; active project detail routes may remain if current sitemap strategy supports detail URLs; the active AI Workflow sub-detail `/[lang]/services/ai-workflow/ai-dev-flow` is included in sitemap/static generation/metadata/language-switch handling
...
- [ ] AI Workflow methodology sub-detail (`/[lang]/services/ai-workflow/ai-dev-flow`)
...
Standalone `/[lang]/services` index, standalone `/[lang]/projects` index, standalone `/[lang]/about` page, retired `/[lang]/services/web3-blockchain`, retired Carina project proof/page, and any `/[lang]/projects/ai-dev-flow` route are not V1 requirements.
...
- [ ] AI Workflow detail representative examples include `Agentic AI 開発ワークフロー` with proof `役割設計 / レビューゲート / 自己進化` and route `/[lang]/services/ai-workflow/ai-dev-flow`
- [ ] AI Workflow `ai-dev-flow` sub-detail explains the multi-agent product delivery workflow/methodology from opportunity/product planning through UX/IA, architecture, PM orchestration, implementation, QA, milestone acceptance, and iteration
- [ ] AI Workflow `ai-dev-flow` sub-detail must not read as a generic project detail, Home Project route, or AI-Driven Development proof page
...
- [ ] Home Projects cards, project detail pages, and service sub-detail representative examples represent the active proof set across:
  - Workflow cases
  - AI application cases
  - AI-driven development cases
- [ ] Japan Life Navi must be represented as the primary AI Application proof point
- [ ] Rigel must be represented as the primary AI-Driven Development proof point where Home/Projects/service proof surfaces need one primary category; it may also mention AI Application product traits as secondary context
- [ ] Astra must be represented as AI Workflow / PMO / project-management workflow proof and must not be shown as the primary AI-Driven Development proof
- [ ] `Agentic AI 開発ワークフロー` must be represented as AI Workflow methodology proof at `/[lang]/services/ai-workflow/ai-dev-flow`, not as a project route
...
- [ ] The `ai-dev-flow` page keeps the same Service detail page tone / `design/DESIGN_SYSTEM.md` / Light Premium Bento baseline; it must not introduce a new palette, standalone design language, or AI-neon/crypto-style visual treatment
- [ ] Text-heavy workflow visuals on `ai-dev-flow` are implemented as code-generated SVG/HTML/CSS diagrams, not AI-generated text images; optional hero imagery, if later approved, is abstract/non-text and visually consistent with existing service pages
...
- [ ] `/[lang]/services/ai-workflow/ai-dev-flow` is generated through static-export-compatible route/static params and does not require middleware, SSR-only runtime behavior, server redirect, or a compatibility page
...
## Could-have
- [ ] Additional methodology sub-detail pages beyond `ai-dev-flow` in a later phase
...
- `/[lang]/projects/ai-dev-flow` or any Projects-namespace version of the AI Workflow methodology page
- Next.js middleware redirects or server redirects for static-export route cleanup, including `/[lang]/projects` index removal or `ai-dev-flow` route compatibility
- Blog / news
- CMS
...
- **Route discipline**: Preserve `/[lang]/services/{serviceId}` detail pages, add `/[lang]/services/ai-workflow/ai-dev-flow` only as an AI Workflow service sub-detail, and preserve active `/[lang]/projects/{slug}` detail pages while deleting/stopping export for `/[lang]/services` and `/[lang]/projects` indexes, retiring only the old `web3-blockchain` detail/content path, and keeping Carina inactive
